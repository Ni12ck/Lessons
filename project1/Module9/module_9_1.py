...
# 2. Возвращать словарь, где ключом будет название вызванной функции, а значением - её результат работы со
# списком int_list.
...
# 2. При попытке передачи, например, списка из строк, некоторые функции могут работать некорректно или вовсе
# не работать. Используйте списки чисел.


# Создал функцию apply_all_func
def apply_all_func(int_list, *functions):
    # Словарь result, где ключом будет название вызванной функции, а значением - её результат работы со
    # списком int_list
    result = {}
    # Функции (*functions) вызываются к переданному списку (int_list)
    for func in functions:
        # Результаты выполнения функций
        res_func = func(int_list)
        # Присвоение имени функции результату этой же функции
        result[func.__name__] = res_func
    # Возвращает словарь result
    return result


# Примеры работы кода:
print(f'Результаты работы функций max и min: {apply_all_func([6, 20, 15, 9], max, min)}')
print(f'Результаты работы функций len, sum, sorted: {apply_all_func([6, 20, 15, 9], len, sum, sorted)}')
