...
# Создал пользовательский класс исключения StepValueError, который наследуется от ValueError
class StepValueError(ValueError):
    pass


# Создал класс Iterator
class Iterator:
    # Атрибуты объекта:
    ...
    # Метод __iter__, который сбрасывает значение pointer на start и возвращающий сам объект итератора
    def __iter__(self):
        self.pointer = self.start
        return self

    # Метод __next__, который увеличивает атрибут pointer на step. В зависимости от знака атрибута step итерация
    # завершится либо когда pointer станет больше stop, либо меньше stop
    def __next__(self):
        result = self.pointer
        # При возвышении и нисхождении по значениям останавливаемся как только доходим до значения остановки итерации
...
print('Результат примера 2:')
for i in iter2:
    print(i, end=' ')
print()
print('Результат примера 3:')
for i in iter3:
    print(i, end=' ')
print()
print('Результат примера 4:')
for i in iter4:
    print(i, end=' ')
print()
print('Результат примера 5:')
for i in iter5:
    print(i, end=' ')
print('-')
